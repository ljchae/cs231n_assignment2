...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #store cache
        cache = (sample_mean, sample_var, x_minus_mean, sqrt_var, inv_var, x_hat, gamma, beta, eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    sample_mean, sample_var, x_minus_mean, sqrt_var, inv_var, x_hat, gamma, beta, eps = cache
    N, D = dout.shape
    ...
    dinv_var = np.sum(dx_hat * x_minus_mean, axis=0)
    dx_minus_mean1 = dx_hat * inv_var

    dsqrtvar = dinv_var * (-1 / (sqrt_var**2))
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1/N) * inv_var * (N * dx_hat - np.sum(dx_hat, axis=0) - x_hat * np.sum(dx_hat * x_hat, axis=0)) 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
